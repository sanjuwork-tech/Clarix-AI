...
function SurveyModal({ onClose }: { onClose: () => void }) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    toast({
      ...
      description: "Thank you! Your feedback will shape FinAI CA.",
    });
  };
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          {...register("name")}
                          placeholder="Bhargavi Sharma"
                          className="w-full px-4 py-3 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm"
                        />
                    ...
                    {watchQualification === "Other" && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
                        ...
                        <input
                          ...
                          placeholder="Your role or qualification..."
                          className="w-full px-4 py-3 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm"
                        />
                      </motion.div>
                    )}
                      ...
                      {SERVICES.map((service) => {
                        ...
                        return (
                          <motion.button
                            ...
                            whileTap={{ scale: 0.97 }}
                            className={`text-left p-4 rounded-xl border-2 transition-all text-sm font-medium flex items-start gap-3 ${
                              checked
                  ...
                  >
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1.5">
                        What's the biggest challenge in your CA / finance journey? <span className="text-destructive">*</span>
                      </label>
                      <textarea
                        {...register("ideas")}
                        placeholder="e.g. I struggle with keeping up with GST amendments, remembering AS/Ind-AS standards, managing audit deadlines... I wish there was an AI tool that could..."
                        rows={5}
                        className="w-full px-4 py-3 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm resize-none"
                      />
                    ...
                    <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-sm text-muted-foreground">
                      ...
                      <p>Your response helps Bhargavi build AI tools that actually solve real CA problems — not imaginary ones. Thank you!</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="flex justify-between mt-8 pt-4 border-t border-border">
                ...
                {step < totalSteps ? (
                  <button
                    type="button"
                    onClick={nextStep}
                    className="px-6 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm flex items-center gap-2 hover:shadow-lg hover:shadow-primary/20 hover:-translate-y-0.5 transition-all"
                  >
                  ...
                  <button
                    ...
                    className="px-8 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm flex items-center gap-2 hover:shadow-lg hover:shadow-primary/20 hover:-translate-y-0.5 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {isSubmitting ? (
